...
class SellerAdService {
  ...
  Future<List<SellerAdModel>> getAds({AdStatus? status}) async {
    log('📢 Fetching ads (DUMMY DATA) - Status: ${status?.name ?? "all"}');

    // Simulate API delay
    await Future.delayed(const Duration(milliseconds: 500));

    var ads = _dummyAds.map((json) => SellerAdModel.fromJson(json)).toList();
  ...
  Future<SellerAdModel?> getAdById(String id) async {
    log('📢 Fetching ad by ID (DUMMY DATA): $id');

    await Future.delayed(const Duration(milliseconds: 300));

    try {
      final adJson = _dummyAds.firstWhere((ad) => ad['id'] == id);
      return SellerAdModel.fromJson(adJson);
    } catch (e) {
      return null;
    }
  }

  /// Create new ad (DUMMY - will be replaced with API call)
  Future<SellerAdModel> createAd({
    ...
    log('📢 Creating ad (DUMMY DATA): $title');

    await Future.delayed(const Duration(milliseconds: 800));

    final newAd = SellerAdModel(
    ...
    // Add to dummy data
    _dummyAds.insert(0, newAd.toJson());

    return newAd;
  }

  /// Update ad (DUMMY - will be replaced with API call)
  Future<SellerAdModel> updateAd(
    String id,
    Map<String, dynamic> updates,
  ) async {
    log('📢 Updating ad (DUMMY DATA): $id');
    ...
    if (index != -1) {
      ...
      return SellerAdModel.fromJson(_dummyAds[index]);
    }
  ...
  Future<Map<String, dynamic>> getAdStats() async {
    log('📢 Fetching ad stats (DUMMY DATA)');

    await Future.delayed(const Duration(milliseconds: 300));

    final ads = _dummyAds.map((json) => SellerAdModel.fromJson(json)).toList();
