...
import 'package:wood_service/views/Buyer/Cart/cart_checkout_bottom_sheet.dart';
...
class _QuantityStepperState extends State<QuantityStepper> {
  ...
  void didUpdateWidget(covariant QuantityStepper oldWidget) {
    ...
    // Always sync with parent widget's quantity to handle cart reloads
    if (_quantity != widget.quantity) {
      setState(() {
        _quantity = widget.quantity;
      });
    }
...
void _removeItemWithUndo(
  ...
  try {
    ...
    final itemToRestore = BuyerCartItem(
      ...
      selectedVariants: removedItem.selectedVariants,
    ...
    final snackBar = SnackBar(
      ...
      action: SnackBarAction(
        ...
        onPressed: () async {
          try {
            final productType =
                itemToRestore.product?.productType ?? 'Ready Product';
            await cartViewModel.addToCart(
              serviceId: itemToRestore.serviceId ?? itemToRestore.productId,
...
void showOutOfStockDialog(
  List<BuyerCartItem> outOfStockItems,
  BuildContext context,
                    ...
                    onPressed: () async {
                      try {
                        ...
                        for (final item in outOfStockItems) {
                          await cartViewModel.removeFromCart(item.id);
                        }
                        Navigator.pop(ctx);
...
void proceedToCheckout(BuildContext context, BuyerCartViewModel cartViewModel) {
  final outOfStockItems = cartViewModel.outOfStockItems;

  if (outOfStockItems.isNotEmpty) {
    showOutOfStockDialog(outOfStockItems, context, cartViewModel);
    return;
  }
